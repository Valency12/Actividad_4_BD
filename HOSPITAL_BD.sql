...
FROM (
    SELECT d.doctor_id, d.full_name, d.tipo_medico,
           ...
           SUM(CASE WHEN DATEDIFF(v.end_date, v.start_date) + 1 < 3 THEN 1 ELSE 0 END) AS periodos_menos_3_dias
    FROM doctors d
    JOIN vacations v 
        ON v.person_type = 'doctor' AND v.person_id = d.doctor_id
    WHERE d.tipo_medico IN ('interino','sustituto') 
      AND YEAR(v.start_date) = 2024
    GROUP BY d.doctor_id, d.full_name, d.tipo_medico
) AS t
WHERE t.total_periodos_2024 = 3 
  AND t.periodos_menos_3_dias >= 1
...
SELECT d.doctor_id, d.full_name, MONTH(v.start_date) AS mes
FROM doctors d
JOIN vacations v 
    ON v.person_type = 'doctor' AND v.person_id = d.doctor_id
WHERE d.tipo_medico = 'sustituto'
  AND YEAR(v.start_date) = 2024
ORDER BY d.full_name, mes;
